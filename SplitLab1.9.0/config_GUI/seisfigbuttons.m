...
uipushtool(ht,'CData',icon.open,...
    ...
    'ClickedCallback', 'db_handle = findobj(''Name'',''Database Viewer''); if ishandle(db_handle); uistack(db_handle,''top''); else; SL_databaseViewer; end;');
...
uipushtool(ht, 'CData', icon.back,...
    ...
    'ClickedCallback', 'idx = thiseq.index-1; if idx < 1; idx = length(eq); end; SL_databaseViewer( ''previous'' ); SL_SeismoViewer(idx)',...
    'BusyAction','Cancel' );
...
uipushtool(ht,'CData',icon.next,...
    ...
    'ClickedCallback','idx = thiseq.index+1; if idx > length(eq); idx =1 ;end; SL_databaseViewer( ''next'' ); SL_SeismoViewer(idx)',...
    'BusyAction','Cancel');
