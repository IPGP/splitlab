...
uipushtool(ht,'CData',icon.open,...
    ...
    'ClickedCallback', 'db_handle = findobj(''Name'',''Database Viewer''); if ~isempty(db_handle); uistack(db_handle,''top''); else; SL_databaseViewer; end; clear db_handle');
...
uipushtool(ht,'CData',icon.home,...
    'TooltipString','Zoom to selected phase [Home or fn+control+leftarrow (Mac)]',...
