...
if ~isempty( fig_handle )
    ...
    for k=10:-1:1;
        ...
        hndfil(k)=uicontrol('Style','checkbox','Tag',sprintf('checkbox%s',k),'Position',[x(5)+15 y 20 16], 'value',config.filterset(k,5), 'Parent',panel,'Callback',['config.filterset(' num2str(k) ',5)=get(gcbo,''Value'');' ]);
    end
    ...
    hnd(7)  = uicontrol('Style','PopUpMenu','Position',[130 165 85 20],'String', {'Seconds' 'Percent' ,'P-window'},'Parent',bpanel,'Value',config.batch.WindowMode,'BackGroundColor','w','Callback',{@activate,'config.batch.WindowMode=get(gcbo,''Value'');'});
