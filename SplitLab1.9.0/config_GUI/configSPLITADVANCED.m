...
fig=figure( 'tag','Advanced','NumberTitle','off','WindowStyle','normal',...
    'name','Advanced Splitting Options', 'Color', [224   223   227]/255,...
    'MenuBar','None','Position', [100 100 500 320]);
...
panel = uipanel('Units','pixel','Title','Filter Key-Press Associations',...
    'Position',[10 10  250 305], 'BackgroundColor', [224   223   227]/255);
...
for k=10:-1:1;
    ...
    hndfil(k)=uicontrol('Style','checkbox','Position',[x(5)+15 y 20 16], 'value',config.filterset(k,5), 'Parent',panel,'Callback',['config.filterset(' num2str(k) ',5)=get(gcbo,''Value'');' ]);
end
   y=pos(4)-(23*(1))-25;
...
uicontrol('Style','edit',...
    'Position',[x(2) 5 50 18],... 
    'String',num2str(config.filter.taperlength),...
    'Parent',panel,...
    'BackGroundColor','w',...
    'Callback', 'config.filter.taperlength=str2num(get(gcbo,''String''));' );
    


%% BATCH Section
bpanel = uipanel('Units','pixel','Title','Batch Processing',...
    'Position',[270 60  220 255], 'BackgroundColor', [224   223   227]/255);




hnd(1) = uicontrol('Style','text','Position',[10 165 120 15], 'String','Units', 'Parent',bpanel);
...
hnd(4) = uicontrol('Style','text','Position',[10  95 120 15], 'String','No. of Begin Windows', 'Parent',bpanel);
hnd(5) = uicontrol('Style','text','Position',[10  75 120 15], 'String','No. of End Windows', 'Parent',bpanel);
hnd(6) = uicontrol('Style','text','Position',[10  55 120 15], 'String','Windowing weight', 'Parent',bpanel);


hnd(7) = uicontrol('Style','PopUpMenu',...
    'Position'       ,[130 165 85 20],...
    'String'         , {'Seconds' 'Percent' ,'P-window'},...
    'Parent'         ,bpanel,...
    'Value'          ,config.batch.WindowMode,...
    'BackGroundColor','w',...
    'Callback'       ,{@activate,'config.batch.WindowMode=get(gcbo,''Value'');'});
...
%%
 ...
 uicontrol('Style','text','Position',[10  25 200 15], 'String','Determine best measurement from', 'Parent',bpanel);
...
hfilter = uicontrol('Style','checkbox','Position' ,[20 215 178 15],'String'   , 'Use multiple Filters','Parent',bpanel,...
...
hwindow= uicontrol('Style','checkbox','Position' ,[20 195 178 15],'String'   , 'Use multiple Windows','Parent',bpanel,...
