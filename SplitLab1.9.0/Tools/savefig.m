...
%	device.png.cmyk=	sprintf(cmdEnd,	'png???', 					[fname '.png']);
	...
	switch(computer)													% Get gs command.
		...
		case {'PCWIN','PCWIN64'},
            if strcmp('SEALAB', getenv('COMPUTERNAME') )
                gs = 'C:\Programme\gs\gs8.61\bin\gswin32c.exe';
            elseif strcmp('WUESTE-G10A', upper(getenv('COMPUTERNAME') ))
                gs= 'C:\Progra~1\ghost\gs8.60\bin\gswin32c.exe';
            end
		otherwise,						gs= 'gs';
	end
