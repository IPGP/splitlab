...
if isfield(thiseq.results,'filter')
    if ~isempty(thiseq.results(end).filter)
        ...
        if length(thiseq.filter)<3; thiseq.filter(3)=2;end %add number of poles; version compatibility
    else
        thiseq.filter =[0 inf 2];  % Hz; cut freqency;  here unfiltered; last entry is number of poles;
    end
else
    thiseq.filter =[0 inf 2];  % Hz; cut freqency;  here unfiltered; last entry is number of poles;
end
...
if isempty(seisfig)
    ...
    width= pos(3)-40 ; height=pos(4)/1.75;
    xpos = pos(1) +20;  ypos = pos(4)/3 ;
...
status = uicontrol('style', 'Text','BackGroundColor', 'w',...
    'Units', 'pixel','Position', [100 2 figpos(3)-85 14],...
    'Parent',seisfig,...
...
if config.calcphase && strcmp(config.studytype,'Teleseismic')
    ...
    set(s,'String', 'Status:  calculating phase arrivals ...'); drawnow
    ...
    if isempty(thiseq.phase)
        ...
        val = strmatch(oldphase, thiseq.phase.Names); %#ok
        if isempty(val);
            val = 1;
        else
            val = val(1);
...
if isfloat(thiseq), if thiseq>0, SL_SeismoViewer(thiseq);end; return,end %
...
for i=1:3
    subax(i) = axes('units', 'normalized', 'Position', pos(i,:), 'box', 'on',...
                    'FontSize', fontsize, 'XMinorTick', 'On', 'Parent', seisfig);
    seis(i)  = plot(thiseq.Amp.time, thiseq.Amp.time, 'Color', scol(i,:), 'Tag', 'seismo', 'Parent', subax(i)); %dummy seismogram, initialises size
end
...
title(subax(4), titlestr, 'visible', 'on', 'FontName', 'FixedWidth', 'Tag', 'BackgroundSeisAxesTitle', 'HandleVisibility', 'on', 'FontSize', fontsize+2)
...
v=version;if sscanf(v(1:3),'%f')>=7.4; set(seisfig, 'WindowScrollWheelFcn',{@figScroll,[seis subax(4)]});end
    ...
    switch get(src,'SelectionType')
        case 'alt' %right click
            ...
            if isfield(thiseq, 'Ppick') && ~isempty(thiseq.Ppick) && ~all(thiseq.Ppick==0)
                ...
                set(status, 'String', sprintf('S-Window: %11.3fsec -- %.3fsec;  vp/vs = %4.3f', split.x(1), split.x(3), vp2vs )); drawnow
            end
        case 'extend' %Shift
            s=nan;
            pointer=[
...
if strcmp(get(src,'SelectionType'),'normal')
    set(split.Ptext, 'position', [split.x(1),1,0], 'Visible', 'on')
    %set(split.hPfill,'Xdata',split.x,'Visible','on','eraseMode','normal');
    set(split.hPfill, 'Xdata', split.x, 'Visible', 'on');
...
set(src, 'WindowButtonUpFcn', {@buttonUp, split, ax, seis})
status = findobj('Tag', 'Statusbar'); set(status, 'String', sprintf(' P-Window: %11.3fsec -- %.3fsec    (dt=%.3f)',split.x(1), split.x(3), split.x(3)-split.x(1))); drawnow
...
if abs(diff(split.x(2:3))) < diff(xlim)*.005 % 0.5precent of total axes length
    status=findobj('Tag','Statusbar');
    set(status,'String','Time window less than 0.5% of window length! Aborting');drawnow
    if strcmp(get(src,'SelectionType'),'normal')
    ...
    if split.x(1) < xxx(1) || split.x(4) > xxx(2)
        ...
        if ishandle(split.hfill)
            ...
            if strcmp(get(src,'SelectionType'),'normal')
                ...
                set(status,'String',sprintf(' P-Window: %11.3fsec -- %.3fsec    (dt=%.3f)',split.x(1),split.x(3),  split.x(3)-split.x(1) ));drawnow
            else
                ...
                status = findobj('Tag','Statusbar');
                set(status,'String',sprintf(' S-Window: %11.3fsec -- %.3fsec   [Press ENTER to start splitting measurement]',split.x(1),split.x(3)));drawnow
            end
        end
    end
...
%    evnt.Key='uparrow';
%    evnt.Character='dummy';
%    seisKeyPress([],evnt,seis)
...
if thiseq.bazi == -12345;    thiseq.bazi = thiseq.geobazi;end
if thiseq.dis  == -12345;    thiseq.dis  = range;end
