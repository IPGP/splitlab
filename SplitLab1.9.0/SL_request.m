...
global config eq
count_eq = (1:length(eq))';  %to name outfiles of downloaded events
...
filen= fullfile(config.savedir,[config.stnname '_' config.request.format '.req']);
...
d = datenum(date(:,1), date(:,2), date(:,3), date(:,4), date(:,5),date(:,6));
...
switch config.request.format
    ...
    case 'Curl'
        ...
        if isempty(config.request.password)
            prefix = sprintf('curl --digest -o ');
        else
        ...
        message   = sprintf( formatstr, [count_eq, reqstart(:,1:6),reqend(:,1:6)]' ); 
    case 'ASCII table'
