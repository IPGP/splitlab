function [errbar_phi,errbar_t,Ccrit] = geterrorbarsRC(Tcomp, Cmatrix, Cresult)
...
if ndf <=K+1
    disp('  NDF <= K... There is no resolution of the XC 95% confidence region; Continuing')
    errbar_phi = [nan nan];
    errbar_t   = [nan nan];
    Ccrit = Cresult;
else
...
if NDF > norig
    ...
    fname2= mfilename;
    disp([char(10) 'WARNING: get_ndf: Number degrees of Freedom NDF (=' num2str(NDF) ') is larger than NORIG (= ' num2str(norig) ') length of pick window. ' char(10) '   In <a href="matlab:edit ' fname '">' fname2 '>getndf</a>'])
    disp('   Please consider to increase sampling rate or pick longer time window.')
    NDF=norig;
    return
end
